...
\textbf{The net as a typed graph.} The net becomes a bipartite graph whose nodes carry structural attributes (marking tokens and degrees per place; invisibility, degrees, move costs, and self-loop context per transition). Every arc is materialized in \emph{both} directions, with the direction kept as a type. The reverse direction matters because what distinguishes two transitions sharing a label is typically \emph{downstream} of them, for example which branch suffix follows each. With messages flowing only along arc direction, two duplicates with symmetric presets would receive provably identical representations; bidirectional messages let the disambiguating suffix reach each duplicate in two hops (Section~\ref{sec:ablation}).
