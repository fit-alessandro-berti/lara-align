...
\node[stagelab, text=black!60] at (1.05,1.95) {1.\ random initialization};
\node[place] (pa) at (0,0) {};
...
\draw[black!70, fill=black!8] (0.7,-2.45) rectangle +(0.3,0.3);
\node[font=\tiny, text=black!70] at (0.85,-2.3) {$b$};
\draw[black!70, fill=black!8] (1.1,-2.45) rectangle +(0.3,0.3);
\node[font=\tiny, text=black!70] at (1.25,-2.3) {$c$};
\node[font=\small, anchor=east] at (0.6,-2.3) {$\sigma'' =$};
\node[font=\scriptsize, text=black!55, align=center] at (1.05,-3.05)
  {scores $\approx$ uniform: the\\duplicates are indistinguishable};
% panel 2: message exchange and imitation of the exact teacher
\node[stagelab, text=black!60] at (5.35,1.95) {2.\ message exchange $+$ imitation};
\node[place] (pb) at (4.3,0) {};
\fill[black!70] (pb.center) circle (0.7mm);
\node[trans, draw=seriesaqua!70!black, line width=1.1pt] (tb1) at (5.45,0.75) {$b$};
\node[trans] (tb2) at (5.45,-0.75) {$b$};
...
\draw[black!70, fill=black!8] (5.0,-2.45) rectangle +(0.3,0.3);
\node[font=\tiny, text=black!70] at (5.15,-2.3) {$b$};
\draw[black!70, fill=black!8] (5.4,-2.45) rectangle +(0.3,0.3);
\node[font=\tiny, text=black!70] at (5.55,-2.3) {$c$};
\node[font=\small, anchor=east] at (4.9,-2.3) {$\sigma'' =$};
\draw[biflow, dashed, draw=black!50] (5.35,-2.1) -- (5.35,-1.58);
\node[flowlab, anchor=west] at (5.5,-1.84) {messages};
\node[font=\scriptsize, text=black!55, align=center] at (5.35,-3.05)
  {cross-entropy on identities pulls\\the sync scores toward $t_1$};
% panel 3: after training
\node[stagelab, text=black!60] at (9.65,1.95) {3.\ after training};
\node[place] (pc) at (8.6,0) {};
\fill[black!70] (pc.center) circle (0.7mm);
\node[trans] (tc1) at (9.75,0.75) {$b$};
\node[trans] (tc2) at (9.75,-0.75) {$b$};
...
\draw[black!70, fill=black!8] (9.3,-2.45) rectangle +(0.3,0.3);
\node[font=\tiny, text=black!70] at (9.45,-2.3) {$b$};
\draw[seriesblue!75!black, fill=seriesblue!12] (9.7,-2.45) rectangle +(0.3,0.3);
\node[font=\tiny, text=seriesblue!60!black] at (9.85,-2.3) {$c$};
\node[font=\small, anchor=east] at (9.2,-2.3) {$\sigma'' =$};
\node[font=\scriptsize, text=black!55, align=center] at (9.65,-3.05)
  {the suffix $c$, seen through\\cross-attention, separates them};
% progression arrows
\draw[flow] (2.25,0) -- node[flowlab, above] {gradient\\steps} (3.75,0);
\draw[flow] (6.55,0) -- node[flowlab, above] {more\\epochs} (8.05,0);
\end{tikzpicture}}
\caption{Toy example of how the foundation model is trained, on a duplicate-label pair and the trace $\sigma'' = \langle b, c \rangle$: the randomly initialized model scores both transitions labeled $b$ alike; during training, net and trace representations exchange messages while the cross-entropy on transition identities pulls the synchronous scores toward the transition fired by the exact teacher alignment $\gamma^\ast$; after convergence the checkpoint resolves such duplicates from the trace suffix alone, yielding scores like those of Figure~\ref{fig:decoding}.}
\label{fig:training}
\end{figure}
