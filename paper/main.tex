...
\end{tikzpicture}}
\caption{The LARA foundation model for alignment: trained once, offline, and vocabulary-free, it maps any Petri net and trace pair to move scores and to deviation diagnostics; the move scores in turn drive a constrained, marking-aware greedy decoder whose candidate alignment is fireable by construction. All outputs are fast but uncertified; the exact layer that verifies, certifies, or repairs the candidate appears in Figure~\ref{fig:architecture}.}
\label{fig:outline}
\end{figure}
...
\textbf{Scope of the bounded search.} Exact aligners explore the synchronous product, may revise any decision, and provably return the cheapest alignment \cite{Zelst2018,Dongen2018}; the decoder's search explores model moves only, stops at the \emph{first} marking that enables the next event rather than the cheapest one, and never revisits a commitment. It guarantees that the candidate can be replayed, not that it is cheap; this is the failure mode analyzed in Section~\ref{sec:failure} and the reason the exact layer stays in the loop. A precomputed per-net runtime keeps the search cheap, so the neural forward pass dominates fast-mode wall time.
...
\textbf{Threats to validity.} Internally, guided and unguided runs share generator seeds and inputs, and all quality metrics are computed against exact optima; scaling configurations use only 10 samples, so per-cell differences carry sampling noise. Externally, two logs cannot establish generality, although the zero-shot protocol makes the transfer test strict. Construct validity rests on cost equality as the certification rule (Section~\ref{sec:preliminaries}).
...
The main open problem is quality at scale: optimality degrades on nets much larger than the training families. Since guidance currently adds nothing over the unguided decoder on the block-structured benchmark, a future checkpoint trained on that richer generator is an improvement only if its guided results clearly exceed this unguided baseline. Beyond that, neural-guided beam or A* search, ranking paths with the same learned scores, attacks the greedy detours at their root, and multi-perspective, data-aware, and object-centric settings offer natural extensions of the certifying architecture.
