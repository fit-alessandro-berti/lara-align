...
% its outputs
\node[iobox, right=27mm of fm.east, anchor=west, yshift=14.5mm, minimum width=40mm, minimum height=11mm] (scores) {move scores\\sync / log / model moves};
\node[iobox, right=27mm of fm.east, anchor=west, minimum width=40mm, minimum height=15.5mm] (cand) {~\\[4.5mm] candidate alignment $\gamma$\\fireable by construction};
\pic at ($(cand.center)+(0,4.4mm)$) {algfrag};
\node[iobox, right=27mm of fm.east, anchor=west, yshift=-14.5mm, minimum width=40mm, minimum height=11mm] (diag) {deviation diagnostics\\latent regions, cost bounds};
% flows
\draw[flow] (net.east) -- ++(5mm,0) |- (fm.west);
\draw[flow] (trace.east) -- ++(5mm,0) |- (fm.west);
\draw[flow] (fm.east) -- node[flowlab, above, sloped, pos=0.62] {scoring heads} (scores.west);
\draw[flow] (fm.east) -- node[flowlab, above, pos=0.55] {greedy decoder} (cand.west);
\draw[flow] (fm.east) -- node[flowlab, below, sloped, pos=0.62] {router $+$ local experts} (diag.west);
% stage background
\begin{scope}[on background layer]
\node[stagebg, fill=seriesblue!5, draw=seriesblue!40] (learnedbg) [fit=(fm)(scores)(cand)(diag)] {};
\end{scope}
...
\end{tikzpicture}}
\caption{The LARA foundation model for alignment: trained once, offline, and vocabulary-free, it maps any Petri net and trace pair to move scores, to a candidate alignment decoded greedily from the scores and fireable by construction, and to deviation diagnostics. All outputs are fast but uncertified; how the exact layer verifies, certifies, or repairs the candidate is the subject of Figure~\ref{fig:architecture}.}
\label{fig:outline}
\end{figure}
