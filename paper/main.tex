...
equal to optimum cost, hence certified optimal & \textbf{91.0\%} \\
exact transition-sequence match & 61.1\% \\
label-level alignment match & 62.5\% \\
...
equal to optimum cost & 86.9\% & \textbf{91.0\%} & +4.1 \\
exact transition-sequence match & 57.8\% & \textbf{61.1\%} & +3.3 \\
label-level alignment match & 58.8\% & \textbf{62.5\%} & +3.7 \\
mean cost gap & 0.26 & \textbf{0.18} & $-$0.08 \\
duplicate-prefix optimal ($n = 64$) & 68.8\% & \textbf{95.3\%} & \textbf{+26.6} \\
duplicate transition selection & 59.4\% & \textbf{79.7\%} & \textbf{+20.3} \\
\bottomrule
\end{tabular}
\end{minipage}
\end{table}
...
metric & value \\
\midrule
mean absolute error of $\Sigma$ upper bounds vs.\ $\delta$ & 0.51 \\
mean signed error & $-$0.15 \\
Pearson correlation with $\delta$ & \textbf{0.82} \\
mean estimate on fitting traces ($\delta = 0$) & 0.27 \\
mean estimate on deviating traces ($\delta > 0$) & 1.78 \\
deviation detection accuracy (threshold 0.5) & \textbf{89.8\%} \\
mean interval width ($\Sigma$ upper $-$ $\Sigma$ lower) & 1.16 \\
