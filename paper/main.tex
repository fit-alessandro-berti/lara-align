...
\textbf{Approximate conformance checking.} Discounted A* changes the cost function \cite{Boltenhagen2021}; fixed-horizon search commits bounded prefixes \cite{Dongen2017}; tandem repeats compress traces \cite{ReissnerTandem2020}; sliding windows retain promising boundary markings between subtraces \cite{Bogdanov2024}; and subset selection transfers representative alignments by edit distance \cite{FaniSani2020}. Token replay \cite{Rozinat2008,Berti2021}, process-tree decomposition \cite{Schuster2021}, and relaxation labeling \cite{Padro2022} provide further speed--quality trade-offs. LARA instead emphasizes a certification boundary: replay verifies candidates before cost equality certifies them or exact search repairs them. Section~\ref{sec:comparison} compares the five PM4Py approximations above.
\textbf{Neural methods in process mining.} Neural models of event data mostly target prediction, through recurrent networks \cite{Tax2017} and transformers \cite{Bukhsh2021}; a recent survey identifies conformance checking as an open opportunity for AI \cite{Genga2025}. These works do not construct legal, costed alignments against an explicit Petri net; LARA is a concrete instance of that agenda.

\textbf{Learning-augmented search.} Outside process mining, machine learning guides combinatorial optimization \cite{Bengio2021}. LARA is deliberately conservative: learned scores rank moves for a greedy decoder and are never used as admissible heuristics, so the trust model of exact conformance checking is preserved. The gap it addresses is \emph{certifiable neural guidance} for Petri net trace alignment.
...
The scoring heads are trained on synthetic traces paired with synthetic Petri nets. For each trace-net pair, the exact aligner computes an optimal alignment. This alignment is stored as the training target. The target records not only which visible labels are matched or skipped, but also which concrete transition was fired. This distinction is important when several transitions have the same label: the model must learn to choose the right transition identity, not only the right activity name. Labels are hashed as described in Section~\ref{sec:encoding}, so the model cannot solve the task by memorizing a fixed vocabulary.

The generator creates small groups of related examples. It first samples a process pattern and then builds two Petri-net representations and two traces for that pattern. The patterns cover the main cases that make alignment difficult: block-structured behavior, duplicate labels, silent routing, parallel branches, interleavings, and non-free-choice M-patterns. Noise is added to the visible trace level, including deletions, insertions, substitutions, repetitions, swaps, and prefix or suffix truncation. The same noisy trace is then used with equivalent net representations, so the examples test whether the model makes consistent choices across different representations of the same behavior.

After generation, the exact backend aligns every trace-net pair and replay-checks the resulting alignment. A group is discarded if equivalent variants lead to different optimal costs, because such cases would give inconsistent supervision. Train, validation, and test splits are assigned before the related representations are expanded, so variants of the same underlying behavior cannot appear in different splits. The reference corpus contains 2{,}048 training examples, 512 validation examples, and 512 test examples. The test traces contain 3.6 events on average, and their optimal costs range from 0 to 7.
...
An interactive Streamlit workbench exposes the certifying workflow without requiring code. It loads an event log and a supplied or discovered Petri net, summarizes trace variants, and runs fast candidates, progressive certification, or an exact baseline. The live view reports coverage, legality, costs, gaps, repairs, and timing. A trace inspector anchors candidate and exact moves to the same observed events, making move types and duplicate-label transition identities directly comparable while keeping replay, neural, and exact evidence separate. Results can also be exported for downstream analysis (Figure~\ref{fig:tool}).
...
We compared fast LARA with PM4Py 2.7.23.2 implementations of discounted A* \cite{Boltenhagen2021}, tandem-repeat compression \cite{ReissnerTandem2020}, sliding-window search \cite{Bogdanov2024}, fixed-horizon search \cite{Dongen2017}, and subset/edit-distance approximation \cite{FaniSani2020}. Outputs were independently replayed and rescored under unit costs on the 512-example held-out split and the 80-example scaling grid. Approximations had 5 seconds per trace; the 30-second exact reference solved 79 stress cases. Table~\ref{tab:approx} restricts exact-cost rates to legal cases with an exact reference.
...
Every Petri-net approximation beats LARA's exact-cost rate on the short held-out traces; sliding window is perfect because the maximum length (11) fits one 20-event window, so no boundary decision is tested. LARA beats only the subset/edit-distance proxy. On the stress grid, sliding window and tandem repeats are much closer to optimal, while discounted A* is faster and more accurate on aggregate. LARA's narrower advantages are a legal result for every case and, at size 40, lower medians (9.04--10.77 ms) than tandem repeats (12.51--23.76), sliding window (13.69--40.02), and fixed horizon (437.50--1398.70); discounted A* remains faster. This is a speed--quality trade-off: LARA's stress p95/max gaps are 19/44. Greedy decoding is therefore the current bottleneck; neural-guided beam/A* and certifying strong approximate candidates are natural next steps.
...
\textbf{Limitations.} At 40 activities only 30--44\% of LARA candidates remain optimal, and the strongest PM4Py candidates are substantially closer to optimum. Training on larger stress nets may help, but the ablation and comparison point directly to irrevocable greedy decisions; neural-guided beam or A* could reconsider them. Larger logs, GPU batching, process-tree and relaxation-based approximations \cite{Schuster2021,Padro2022}, and richer conformance settings remain open.
...
This paper introduced LARA, in which a vocabulary-free foundation model proposes Petri-net trace alignments and replay plus exact search provides the guarantees. The design is \emph{Learned} through move scores, \emph{Adaptive} to each net--trace pair and assurance mode, and \emph{Recombined} through regional diagnostics. The prototype returns legal alignments across all tested distributions, attains and certifies the in-distribution optimum in 91.0\% of cases, and improves duplicate-prefix optimality from 68.8\% to 95.3\% over unguided decoding. It overtakes exact search on larger and invisible-heavy models. Comparison with PM4Py approximations qualifies this result: the current greedy candidate has lower cost quality than the strongest baselines, but reliable legal output and low large-case latency. LARA's main contribution is therefore the certifying learned architecture; improving its decoder is the clearest next step.
