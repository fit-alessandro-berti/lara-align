...
The diagnostic branch of Section~\ref{sec:architecture} is trained but, unlike the moves, never verified at alignment time, so its usefulness must be established empirically: we compare its output with the exact optimum on the same held-out split, and Table~\ref{tab:costest} summarizes the outcome. As a point estimate, the summed upper bounds track the optimal cost closely and separate fitting from deviating traces reliably: thresholding the estimate at 0.5 decides whether a trace deviates at all with 93\% accuracy, so traces can be ranked by expected deviation before any alignment is computed. As an \emph{interval}, however, the branch is overconfident, predicting narrow ranges that rarely contain the optimum; this miscalibration confirms the design decision of Section~\ref{sec:architecture} that learned bounds inform the analyst and never enter the search.
...
The main open problem is quality at scale: optimality degrades on nets much larger than the training families. The assessment also provides a clear criterion for progress on this problem, since guidance currently adds nothing over the unguided decoder on the block-structured benchmark: a future checkpoint, trained on that richer generator, is an improvement only if its guided results clearly exceed this unguided baseline. Beyond that, neural-guided beam or A* search, ranking paths with the same learned scores, attacks the greedy detours at their root, and multi-perspective, data-aware, and object-centric settings offer natural extensions of the certifying architecture.
