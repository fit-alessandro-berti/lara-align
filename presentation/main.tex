...
\begin{frame}{From Petri Net + Trace to Transition Likelihoods}
  ...
  \begin{tikzpicture}[font=\scriptsize, x=1cm, y=1cm]
    ...
    \node[lrn, minimum width=27mm] (graph) at (3.7,1.55)
      {\textbf{graph transformer}\\3 layers; 4 arc types\\forward + reverse};
    \node[lrn, minimum width=20mm] (trans) at (7.05,1.55)
      {$h_j\in\mathbb{R}^{128}$\\per transition $t_j$};
    % Trace branch
    ...
    \node[lrn, minimum width=27mm] (seq) at (3.7,-1.05)
      {\textbf{trace transformer}\\2 layers; positions\\global trace context};
    \node[lrn, minimum width=20mm] (events) at (7.05,-1.05)
      {$e_i\in\mathbb{R}^{128}$\\per event $a_i$};
    % Coupling and output
    \draw[flow] (net) -- (graph);
    ...
    \node[lrn, minimum width=31mm, minimum height=21mm] (score) at (10.7,0.25)
      {\textbf{label-masked}\\\textbf{bilinear score}\\[1mm]
       $s_{ij}=\dfrac{(W e_i)^\top h_j}{\sqrt{128}}$\\
...
\begin{frame}{Why Consider LARA in Process Mining?}
  \begin{block}{When exact alignment is the bottleneck, LARA changes the
    speed--trust trade-off without changing what an alignment means.}
    \centering
  ...
  \begin{column}{0.31\textwidth}
    \centering \keyc{Faster interaction}\par
    \vspace{1mm}\small
    A near-fixed CPU forward pass avoids the heavy tail of exact search;
    the fast path overtakes A* on larger models and already helps on an
    invisible-heavy discovered net.
  \end{column}
  \begin{column}{0.31\textwidth}
  ...
  \end{column}
  \begin{column}{0.31\textwidth}
    \centering \keya{Low adoption risk}\par
    \vspace{1mm}\small
    One vocabulary-free checkpoint applies without per-log retraining;
    certification can be requested whenever optimality matters, and failures
    fall back to exact repair.
  \end{column}
  \end{columns}
  \vspace{5mm}
  \centering
  \textbf{Practical role:} a safe accelerator in front of the exact aligner,
  \keya{not a replacement for formal guarantees}.
\end{frame}
...
\begin{frame}{Appendix: In-Distribution Results}
  ...
  \begin{column}{0.46\textwidth}
    ...
    \begin{tabular}{lr}
      \toprule
      Held-out test split ($n = 100$) & \\
      \midrule
      replayable (legal) & \keyg{100.0\%} \\
      certified optimal & \keyc{88.0\%} \\
      exact transition-sequence match & 68.0\% \\
      mean / median cost gap & 0.38 / 0.00 \\
      p95 / max cost gap & 4.0 / 6.0 \\
      \bottomrule
    \end{tabular}
    \end{center}
    \vspace{3mm}
    \small All non-zero gaps are \keya{even}: the signature of the greedy
    decoder trading $k$ synchronous moves for $k$ model $+$ $k$ log moves,
    a penalty of exactly $2k$.
  \end{column}
  \begin{column}{0.5\textwidth}
    \begin{center}
    \begin{tikzpicture}
    \begin{axis}[
      ...
      xmin=-0.7, xmax=6.7, ymin=0, ymax=100,
      xtick={0,1,2,3,4,5,6},
    ...
    \addplot[fill=larablue, draw=none] coordinates {(0,88)(2,6)(4,5)(6,1)};
    \end{axis}
    \end{tikzpicture}
    \end{center}
  \end{column}
...
\begin{frame}{Appendix: Guidance Ablation}
  ...
  \begin{tabular}{lrrr}
    \toprule
    Metric ($n = 100$) & unguided & guided & $\Delta$ \\
    \midrule
    replayable & 100.0\% & 100.0\% & 0.0 \\
    equal to optimum cost & 83.0\% & \keyc{88.0\%} & $+$5.0 \\
    exact transition-sequence match & 63.0\% & \keyc{68.0\%} & $+$5.0 \\
    mean cost gap & 0.48 & \keyc{0.38} & $-$0.10 \\
    sequence family, optimal ($n = 82$) & 86.6\% & 86.6\% & 0.0 \\
    duplicate-label family, optimal ($n = 18$) & 66.7\% & \keyc{94.4\%} & \keyg{$+$27.7} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{3mm}
  \begin{itemize}\setlength{\itemsep}{4pt}
    \item On \keyc{sequence} Petri nets every event has at most one matching
      transition, so guidance changes \keya{nothing}
    \item The \keyg{entire} in-distribution contribution of learning sits in
      \keya{duplicate-label} identity resolution, exactly as the mechanism predicts
  \end{itemize}
\end{frame}
...
\begin{frame}{Appendix: Scaling Benchmark}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \begin{tabular}{rrrrr}
      \toprule
      size & legal & optimal & fast med. & exact med. \\
      \midrule
       5 & 100\% & 80\% & 14.9 ms & 1.0 ms \\
      10 & 100\% & 80\% &  7.9 ms & 1.4 ms \\
      20 & 100\% & 60\% &  9.9 ms & 2.2 ms \\
      40 & 100\% & 30\% & 12.1 ms & \keya{44.0 ms} \\
      \bottomrule
    \end{tabular}
    \end{center}
    \vspace{2mm}
    \small (deviation rate 0.15 shown)\\[2mm]
    \keyg{Legality is scale-invariant}; the optimal-cost rate
    \keya{drops} out of distribution, while the fast path stays at the
    \keyc{forward-pass floor}.
  \end{column}
  \begin{column}{0.46\textwidth}
    \begin{center}
    \begin{tikzpicture}
    ...
    \addplot[larablue, mark=*, mark size=1.5pt]
      coordinates {(5,14.9)(10,7.9)(20,9.9)(40,12.1)};
    \addlegendentry{LARA fast}
    \addplot[laraaqua, mark=square*, mark size=1.4pt]
      coordinates {(5,1.0)(10,1.4)(20,2.2)(40,44.0)};
    \addlegendentry{exact A*}
    \end{axis}
    \end{tikzpicture}
    \end{center}
    \small Crossover at about \keyc{40 activities}; first exact
    \keya{timeouts} appear there.
  \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Appendix: Zero-Shot Results on Real-Life Logs}
  ...
  \begin{tabular}{llrrrr}
    ...
    \midrule
    road traffic & 0.0 & \keyg{100\%} & 100.0\% & 0 & 8.0 / 1.7 ms \\
    road traffic & 0.5 & \keyg{100\%} & 94.0\%  & 3 & 7.9 / 1.3 ms \\
    receipt      & 0.0 & \keyg{100\%} & 82.6\%  & 5 & \keyc{18.5 / 27.4 ms} \\
    receipt      & 0.5 & \keyg{100\%} & 74.6\%  & 7 & 8.9 / 8.1 ms \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{3mm}
  \begin{itemize}\setlength{\itemsep}{5pt}
    ...
    \item Optimality is strong on \keyc{frequent} behavior; misses concentrate
      in \keya{rare, long variants} and remain safe upper bounds
    \item On the largest discovered Petri net, the fast path \keyc{beats exact
      A*} on a real log (18.5 vs.\ 27.4 ms median)
  \end{itemize}
\end{frame}
