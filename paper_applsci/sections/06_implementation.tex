...
The repository includes the reference synthetic corpus, checkpoint, generation configuration, per-epoch metrics, and benchmark outputs. The script \texttt{init\_data.py} generates the behavior families; \texttt{train\_model.py} trains and saves checkpoints. Evaluation scripts cover the fixed synthetic test split, scaling inputs, PM4Py approximations, and real logs. The journal source additionally packages the result data used by its plots and records hashes of the source artifacts, so figures can be rebuilt without rerunning training.
