...
Subset/edit distance & Conformance estimates and bounds & Representative model executions & Reuses behavior for a process.\\
...
LARA & Candidate verified by replay; optional exact result & One pretrained Petri net--trace scorer & Machine learning ranks; semantics checks.\\
\bottomrule
\end{tabularx}
\end{table*}
...
Approximation methods exploit different sources of regularity. Fixed horizon alignment combines an executable prefix with a marking equation estimate of the remaining behavior before committing~\cite{Dongen2017}. Discounted alignment changes the influence of later deviations through a discounted objective~\cite{Boltenhagen2021}. Sliding window methods divide long traces into smaller search problems and retain selected boundary candidates, using global structural information about the remaining trace to rank them~\cite{Bogdanov2024}. These methods reduce search effort through local commitments or a modified objective, which can yield a higher cost under the original objective. LARA also makes bounded search decisions; its learned ranking uses the complete trace as context.
...
Subset selection and edit distance approximation estimate conformance values and provide upper and lower bounds using a subset of model behavior~\cite{FaniSani2020}. This behavior can be obtained by aligning selected log traces or by simulating the model. The reusable object is therefore a collection of representative model executions, whereas LARA reuses neural network parameters learned over synthetic Petri nets. The PM4Py variant evaluated here additionally retains complete transition sequences to reconstruct executable candidate alignments, including invisible transitions. Representative executions must cover relevant behavior, while a pretrained scorer must generalize to the target structure and observation patterns. Our subset comparison uses the concrete configuration in Section~\ref{sec:approximations}; it does not establish a best possible performance for all subset sizes or representative selection policies.
...
Neural process mining research provides useful representations but often solves a different task. Recurrent models carry a hidden state from one event to the next; predictive monitoring uses this state to estimate future events, timestamps, or remaining execution time from prefixes~\cite{Tax2017}. ProcessTransformer applies transformer based sequence modeling to predictive monitoring~\cite{Bukhsh2021}. Such predictions do not, by themselves, identify an executable minimum deviation explanation against a given Petri net. LARA has the complete trace available and returns an alignment whose transition identities can be replayed.
...
The foundation model perspective encourages pretraining a shared representation for reuse. Bommasani et al.~\cite{Bommasani2021} discuss this perspective across tasks and modalities, while Rizk et al.~\cite{Rizk2024} argue for models tailored to business process data. The present study takes a limited step in that direction: one scorer is reused across alignment inputs. Its evidence concerns synthetic motifs and a small number of target logs, so it does not establish a general purpose foundation model for process mining. This qualification is necessary to align the terminology with the scale and tasks actually evaluated.
