...
\end{align}
The two contexts are calculated from the vectors before cross attention. An event can now reflect which transitions are relevant in this Petri net, and a transition can reflect which events occur in this trace. These numerical associations explain how the neural network can use the suffix $c$ to prefer one $b$ transition; they do not guarantee that it learns the appropriate association on every input.
...
\textbf{Processing.} Three output heads convert these vectors into preferences for the move vocabulary. With learned parameters $W,w_M,w_L,b_M,b_L$, the scores are
\begin{align}
...
\textbf{Input.} The decoder receives $SN$, $\sigma$, the fixed score arrays $S,M$, and two search limits: a depth $d_p$ for enabling the next event and a depth $d_f$ for reaching the final marking after all events. The defaults are $d_p=8$ and $d_f=32$; the stress and real log experiments use $d_f=64$.
...
In \Fast{} mode the system returns the candidate, replay status, and verified cost when available. In \Certified{} mode it additionally calls PM4Py's state equation A* aligner. If an optimal exact result is available, it retains a legal candidate with the same cost; otherwise it returns the exact alignment as a replacement, with replay checks on that result. If exact search times out or fails, a legal candidate remains an upper bound and optimality stays unconfirmed.
