...
\textbf{Input.} The representation stage receives the system net $SN=(P,T,F,\lambda,m_0,m_f)$, the complete trace $\sigma$, and the move costs from Table~\ref{tab:moves}. The numbers of places, transitions, and events may vary between inputs.
...
\textbf{Output.} The stage returns the tuple $X$ in Equation~\eqref{eq:input}, together with the ordered identifiers that link tensor rows to concrete places and transitions. The neural network receives structural features and label encodings; the decoder retains the original labels and net semantics for exact checks.
...
\end{align}
The two contexts are calculated from the vectors before cross attention. An event can now reflect which transitions are relevant in this net, and a transition can reflect which events occur in this trace. These numerical associations explain how the neural network can use the suffix $c$ to prefer one $b$ transition; they do not guarantee that it learns the appropriate association on every input.

\textbf{Output.} The encoders return contextual event vectors $V\in\R^{n\times d}$ and transition vectors $U\in\R^{|T|\times d}$, whose rows are $v_i$ and $u_j$. These vectors summarize the supplied net and complete trace for the scoring stage; they do not represent the decoder's changing marking.
...
\textbf{Input.} The verification stage receives the proposed sequence $\widehat\gamma$, the original net and trace, the move costs, and the requested mode, \Fast{} or \Certified{}. Verification is needed even though decoding fires enabled transitions: bounded completion can fail, and independent checking also detects malformed candidates.
