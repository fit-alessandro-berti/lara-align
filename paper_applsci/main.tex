...
\Title{Learning to Propose, Replaying to Verify: Reusable Neural Guidance for Petri Net Alignments}
...
\dataavailability{The implementation and experiment scripts are available at \url{https://github.com/fit-alessandro-berti/lara-align}. The \texttt{ft-journal} branch includes the reference synthetic corpus, numerical result records, and figure sources in \texttt{paper\_applsci}. The pretrained checkpoint is available at \url{https://www.alessandroberti.it/checkpoint_lara_latest.tar.gz}; the journal source records its SHA-256 checksum. The public event-log sources are cited in Section~\ref{sec:real}.}
\acknowledgments{Generative AI tools supported software implementation and manuscript preparation. OpenAI Codex assisted with restructuring the text, explaining the methodology, preparing figure code, checking references, and compiling the manuscript. The authors take responsibility for the content.}
